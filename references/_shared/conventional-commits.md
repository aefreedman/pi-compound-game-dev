...
|------|-------------|---------|
| **feat** | New feature | `feat(player): add dash mechanic` |
| **fix** | Bug fix | `fix(inventory): prevent item duplication` |
| **refactor** | Code restructuring (no behavior change) | `refactor(camera): extract follow logic` |
| **perf** | Performance improvement | `perf(rendering): reduce overdraw in arena` |
| **test** | Adding or updating tests | `test(save): add migration edge cases` |
...
| **chore** | Build process or auxiliary tool changes | `chore(unity): update package lock` |
| **ci** | CI/CD configuration changes | `ci(build): add editmode tests` |
...
**Good examples:**
- `feat(player): add wall jump`
- `fix(save): handle missing slot data`
- `refactor(camera): extract shake logic`

**Bad examples:**
- `feat(player): Added wall jump` (not imperative)
- `feat(player): Add Wall Jump` (capitalized)
- `feat(player): add wall jump.` (period at end)
...
**Breaking changes:**
```
feat(save)!: change save slot format

BREAKING CHANGE: save files now require schema version 3
```
...
**Attribution:**
```
feat(tools): add spawn table editor
...
Keeps lobby state in sync with the existing network session flow.
...
**Load this file:**
```markdown
Load conventional commit format from ./conventional-commits.md
```
