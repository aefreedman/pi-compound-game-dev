...
Only the root/orchestrator invokes the agent. Use `agentScope: "both"` and the runtime's Pi-trust-aware fallback so project-local package installations remain discoverable without redundant prompts. Follow references/_shared/subagent-execution-profiles.md; inherit by default, then raise or lower thinking from the actual flow ambiguity and evidence burden.
